...
class enrol_saman_testcase extends advanced_testcase {
    ...
    public function test_basics() {
        $this->assertFalse(enrol_is_enabled('saman'));
        $plugin = enrol_get_plugin('saman');
        $this->assertInstanceOf('enrol_saman_plugin', $plugin);
        $this->assertEquals(ENROL_EXT_REMOVED_SUSPENDNOROLES, get_config('enrol_saman', 'expiredaction'));
    }
    ...
    public function test_sync_nothing() {
        ...
        $this->enable_plugin();
        $samanplugin = enrol_get_plugin('saman');

        // Just make sure the sync does not throw any errors when nothing to do.
        $samanplugin->sync(new null_progress_trace());
    }
    ...
    public function test_expired() {
        ...
        /** @var enrol_saman_plugin $samanplugin  */
        $samanplugin = enrol_get_plugin('saman');
        ...
        $samanplugin->enrol_user($instance1, $user1->id, $studentrole->id);
        $samanplugin->enrol_user($instance1, $user2->id, $studentrole->id);
        $samanplugin->enrol_user($instance1, $user3->id, $studentrole->id, 0, $now-60);

        $samanplugin->enrol_user($instance2, $user1->id, $studentrole->id, 0, 0);
        $samanplugin->enrol_user($instance2, $user2->id, $studentrole->id, 0, $now-60*60);
        $samanplugin->enrol_user($instance2, $user3->id, $studentrole->id, 0, $now+60*60);

        $samanplugin->enrol_user($instance3, $user1->id, $teacherrole->id, $now-60*60*24*7, $now-60);
        $samanplugin->enrol_user($instance3, $user4->id, $teacherrole->id);
        ...
        $samanplugin->set_config('expiredaction', ENROL_EXT_REMOVED_KEEP);
        $code = $samanplugin->sync($trace);
        $this->assertSame(0, $code);
        $this->assertEquals(9, $DB->count_records('user_enrolments'));
        $this->assertEquals(9, $DB->count_records('role_assignments'));


        $samanplugin->set_config('expiredaction', ENROL_EXT_REMOVED_SUSPENDNOROLES);
        $samanplugin->sync($trace);
        $this->assertEquals(9, $DB->count_records('user_enrolments'));
        ...
        $samanplugin->set_config('expiredaction', ENROL_EXT_REMOVED_UNENROL);
